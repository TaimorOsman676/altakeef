...
export default function Card({
  ...
  if (gradientBorder) {
    return (
      <motion.div
        className="rounded-2xl p-[2px] bg-gradient-to-br from-[#00E5FF] to-[#1E293B]"
        {...hoverAnimation}
      >
        <div className={`rounded-[14px] ${glass ? 'bg-white/10 backdrop-blur-xl' : 'bg-[#111827]'} ${className}`} {...rest as React.HTMLAttributes<HTMLDivElement>}>
          {children}
        </div>
      </motion.div>
    );
